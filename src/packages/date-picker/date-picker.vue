<template>
  <div class="mc-date-picker" v-click-outside="handleBlur">
    <mc-input suffix-icon="rili" @focus="handleFocus" :value="formateDate" @change="handleChange"></mc-input>
    <div class="mc-date-content" v-if="isVisible">
      <div class="mc-date-picker-content">
        <template v-if="mode === 'dates'">
          <div class="mc-date-picker-header">
            <mc-icon icon="prev" @click="changeYear(-1)"></mc-icon>
            <mc-icon icon="left" @click="changeMonth(-1)"></mc-icon>
            <span>
              <b @click="mode='years'">{{tempTime.year}}</b>年
              <b @click="mode='months'">{{tempTime.month + 1}}</b>月
            </span>
            <mc-icon icon="right" @click="changeMonth(1)"></mc-icon>
            <mc-icon icon="next" @click="changeYear(1)"></mc-icon>
          </div>
          <div>
        ...
        <template v-if="mode === 'years'">
            <div class="mc-date-picker-header">
            <mc-icon icon="prev" @click="changeYear(-10)"></mc-icon>
            <span>
              <b @click="mode='years'">{{startYear}}年{{startYear+9}}年</b>
            </span>
            <mc-icon icon="next" @click="changeYear(10)"></mc-icon>
          </div>
        </template>
        <template v-if="mode === 'months'">
            <div class="mc-date-picker-header">
            <mc-icon icon="left" @click="changeYear(-1)"></mc-icon>
            <span>
              <b @click="mode='years'">{{this.tempTime.year}}年</b>
            </span>
            <mc-icon icon="right" @click="changeYear(1)"></mc-icon>
          </div>
        </template>
      </div>
    </div>
